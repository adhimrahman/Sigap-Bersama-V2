...
export default function SignupIndividu({ handleSignup, setEmail, setPassword, setConfirmPassword, setFullName, email, password, confirmPassword, fullName }) {
    ...
    return (
        <form className="flex flex-col items-center justify-center bg-[#282C34]/95 w-2/5 h-fit rounded-[16px] backdrop-blur-sm gap-5"
            ...
            <div className='flex flex-col w-10/12 gap-3 capitalize'>
                ...
                <input type="text" placeholder="Full Name" value={fullName} name='fullName' onChange={(e) => setFullName(e.target.value)}
                ...
                <button type="submit" className="p-2 h-14 w-full mt-4 bg-[#008080] text-white font-bold rounded-[12px] hover:bg-[#008080]/40">
                    Daftar
                </button>
            </div>
        
            <p className="mt-2 mb-8">Sudah punya akun? 
                <span onClick={() => navigate('/signin')} className='text-blue-500 ml-3 cursor-pointer hover:font-bold'>Login</span>
            </p>
        </form>
    );
}
